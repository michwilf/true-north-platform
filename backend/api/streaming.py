...
async def stream_multi_agent_analysis(
    ...
    try:
        ...
        final_state, final_decision = trading_agents_graph.propagate(
...
async def stream_market_regime_analysis(
    regime_detector, macro_agent, technical_agent, sentiment_agent
) -> AsyncGenerator[Dict[str, Any], None]:
    """
    Stream market regime analysis progressively.
    """
    try:
        yield {"event": "start", "message": "Starting market regime analysis"}

        # Step 1: Get regime data
        yield {"event": "fetching_data", "message": "Fetching market regime data..."}
        ...
        # Step 3: Synthesis
        yield {"event": "synthesis", "message": "Synthesizing insights..."}
...
async def stream_portfolio_analysis(
    portfolio_tracker, portfolio_manager, risk_analyst, performance_analyst
) -> AsyncGenerator[Dict[str, Any], None]:
    """
    Stream portfolio analysis progressively.
    """
    try:
        yield {"event": "start", "message": "Starting portfolio analysis"}
        ...
        for i, agent_data in enumerate(agents):
            ...
            if name == "Portfolio Manager":
                result = await agent.analyze(
                    agent_data[2], agent_data[3], agent_data[4]
                )
            else:
        ...
        # Send final result
        yield {"event": "done", "metrics": metrics, "agent_analyses": agent_results}
