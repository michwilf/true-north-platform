...
async def get_enhanced_portfolio_analysis(
    ...
    **Performance:** First call takes ~15 seconds (3 LLM calls), then cached for 5 minutes.
    ...
    try:
        ...
        print(f"   Daily P&L: ${metrics.get('daily_pnl', 0):+,.2f} ({metrics.get('daily_pnl_percent', 0):+.2f}%)")
        ...
        try:
            ...
            print(f"   Market Regime: {market_regime.get('volatility_regime', 'UNKNOWN')} / {market_regime.get('market_trend', 'UNKNOWN')}")
        ...
        if not risk_factors:
            risk_factors = ["Moderate portfolio risk", "Monitor position sizes", "Review stop-losses"]
        ...
        response_data = {
            ...
            "portfolio_manager_analysis": AgentPerspective(
                ...
                key_insights={
                    ...
                    "key_recommendations": manager_result.get('key_recommendations', []),
                }
            ),
            "risk_analyst_analysis": AgentPerspective(
                ...
                key_insights={
                    "risk_level": risk_result.get('risk_level', 'MEDIUM RISK'),
                    "concentration_metrics": risk_result.get('concentration_metrics', {}),
                    "risk_factors": risk_result.get('risk_factors', []),
                }
            ),
            "performance_analyst_analysis": AgentPerspective(
        ...
        print(f"🎯 Top Recommendation: {top_recommendations[0] if top_recommendations else 'N/A'}")
