    ...
    **Example:**
    ...
    try:
        ...
        if request.risk_levels:
            opportunities = [
                opp for opp in opportunities if opp.risk_level in request.risk_levels
            ]

        # Rank by confidence and technical scores
        opportunities.sort(
            key=lambda x: (x.confidence_level + x.technical_score) / 2, reverse=True
        )
        ...
        if request.analyze_with_agents and top_opportunities:
            ...
            for i, opp in enumerate(top_opportunities, 1):
                try:
                    print(
                        f"   [{i}/{len(top_opportunities)}] Analyzing {opp.symbol}..."
                    )
                    ...
                    current_price = (
                        hist["Close"].iloc[-1] if not hist.empty else opp.price
                    )
        ...
        for r in agent_results[:3]:
            print(
                f"   • {r.symbol}: {r.recommendation} ({r.confidence:.0%} confidence)"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error in autonomous discovery: {str(e)}"
        )
